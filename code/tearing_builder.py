...
Zone 轴与 Gate 轴的滞后相关最大值都在 lag=0，两侧接近 0，
**没有「核心先裂、外围后跟」的传导时序**。

✓ 2026-09-25 复核：两条轴的峰值都在 lag=0，声明成立；0.703 / 0.773 也复现
（gen_paper_doc.py 的 s5.sync，Δdd 口径 + 周度中位序列，实测 0.7035 / 0.7729）。
「两侧接近 0」只在**差分**口径下成立（非零 lag 的 |r| 均值：水平 0.63 / 0.68，
差分 0.058 / 0.057）—— 水平量自相关强，两侧天然也高。该对照已并入 s5.sync，
每次构建重算。具体取值以那里为准，本文档不作第二个出处。
