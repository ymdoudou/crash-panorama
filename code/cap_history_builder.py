...
━━ 为什么 base 必须逐季度算 ━━
把 base 取成单期冻结值看起来省事，代价是给历史用了未来的横截面：
2020 年的 cap 会由 2026 年才存在的那批公司与那时的估值分布推出来。
本文件因此逐季度各算各的 base，每一季只用到该季生效日之前的信息。
两条口径（PE / PS）各有自己的 base 与 R，不互相借用当日快照。

本文件是 cap 的唯一来源，一律经 cap_accessor 读取。
